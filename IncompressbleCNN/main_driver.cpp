...
struct StokesCNNet_Ux : torch::nn::Module {
  StokesCNNet_Ux( )
    :  convf11(torch::nn::Conv3dOptions(1 , 1, 3).stride(1).padding(3).dilation(3).groups(1).bias(false).padding_mode(torch::kZeros)),
       ...
       BNorm3D11(torch::nn::BatchNorm3dOptions(1).affine(true).track_running_stats(true)),
       BNorm3D12(torch::nn::BatchNorm3dOptions(1).affine(true).track_running_stats(true)),
       BNorm3D13(torch::nn::BatchNorm3dOptions(1).affine(true).track_running_stats(true)),
       BNorm3D14(torch::nn::BatchNorm3dOptions(1).affine(true).track_running_stats(true)),
       BNorm3D15(torch::nn::BatchNorm3dOptions(1).affine(true).track_running_stats(true)),
       BNorm3D21(torch::nn::BatchNorm3dOptions(1).affine(true).track_running_stats(true)),
       BNorm3D22(torch::nn::BatchNorm3dOptions(1).affine(true).track_running_stats(true)),
       BNorm3D23(torch::nn::BatchNorm3dOptions(1).affine(true).track_running_stats(true)),
       BNorm3D24(torch::nn::BatchNorm3dOptions(1).affine(true).track_running_stats(true)),
       BNorm3D25(torch::nn::BatchNorm3dOptions(1).affine(true).track_running_stats(true)),
       BNorm3D31(torch::nn::BatchNorm3dOptions(1).affine(true).track_running_stats(true)),
       BNorm3D32(torch::nn::BatchNorm3dOptions(1).affine(true).track_running_stats(true)),
       BNorm3D33(torch::nn::BatchNorm3dOptions(1).affine(true).track_running_stats(true)),
       BNorm3D34(torch::nn::BatchNorm3dOptions(1).affine(true).track_running_stats(true)),
       BNorm3D35(torch::nn::BatchNorm3dOptions(1).affine(true).track_running_stats(true))
    { 
        ...
        register_module("BNorm3D11", BNorm3D11);
        register_module("BNorm3D12", BNorm3D12);
        register_module("BNorm3D13", BNorm3D13);
        register_module("BNorm3D14", BNorm3D14);
        register_module("BNorm3D15", BNorm3D15);

        register_module("BNorm3D21", BNorm3D21);
        register_module("BNorm3D22", BNorm3D22);
        register_module("BNorm3D23", BNorm3D23);
        register_module("BNorm3D24", BNorm3D24);
        register_module("BNorm3D25", BNorm3D25);

        register_module("BNorm3D31", BNorm3D31);
        register_module("BNorm3D32", BNorm3D32);
        register_module("BNorm3D33", BNorm3D33);
        register_module("BNorm3D34", BNorm3D34);
        register_module("BNorm3D35", BNorm3D35);
    }
                            ...
                            int maxDim,const std::vector<int> srctermTensordim, const std::vector<int> umacTensordims)
   {
        ...
        f1 = LRELU(BNorm3D11(convf11(f1.unsqueeze(1))));
        f1 = LRELU(BNorm3D12(convf12(f1)));
        f1 = LRELU(BNorm3D13(convf13(f1)));
        f1 = LRELU(BNorm3D14(convf14(f1)));
        f1 = LRELU(BNorm3D15(convf15(f1)));
        f1 = (convf16(f1));
        f1 = f1.squeeze(1);
        ...
        f2 = LRELU(BNorm3D21(convf21(f2.unsqueeze(1))));
        f2 = LRELU(BNorm3D22(convf22(f2)));
        f2 = LRELU(BNorm3D23(convf23(f2)));
        f2 = LRELU(BNorm3D24(convf24(f2)));
        f2 = LRELU(BNorm3D25(convf25(f2)));
        f2 = (convf26(f2));
        f2 = f2.squeeze(1);
        ...
        f3 = LRELU(BNorm3D31(convf31(f3.unsqueeze(1))));
        f3 = LRELU(BNorm3D32(convf32(f3)));
        f3 = LRELU(BNorm3D33(convf33(f3)));
        f3 = LRELU(BNorm3D34(convf34(f3)));
        f3 = LRELU(BNorm3D35(convf35(f3)));
        f3 = (convf36(f3));
        f3 = f3.squeeze(1);
   ...
   torch::nn::BatchNorm3d BNorm3D11,BNorm3D12,BNorm3D13,BNorm3D14,BNorm3D15;
   torch::nn::BatchNorm3d BNorm3D21,BNorm3D22,BNorm3D23,BNorm3D24,BNorm3D25;
   torch::nn::BatchNorm3d BNorm3D31,BNorm3D32,BNorm3D33,BNorm3D34,BNorm3D35;
...
class CustomDatasetCNN : public torch::data::Dataset<CustomDatasetCNN>
{
    ...
    public:
        CustomDatasetCNN(std::array<torch::Tensor,AMREX_SPACEDIM> SrcTensor, torch::Tensor Pres,
                            ...
                            const std::vector<int> umacTensordims,const std::vector<int> pgTensorDims)
        {
            ...
            /* Compute phi=f-dp/dxi */
            std::array<torch::Tensor,AMREX_SPACEDIM> PhiTensor;
            for (int d=0;d<AMREX_SPACEDIM;d++) 
            {
                PhiTensor[d]= SrcTensor[d]-pgTensor[d];
            }

            /* Add channel dim to ever tensor component of the Std::array<torch::tensor,Amrex_dim> objects */
            for (int d=0;d<AMREX_SPACEDIM;d++) 
            {
                PhiTensor[d]=PhiTensor[d].unsqueeze(1);
            ...
            PhiTensor[0]= torch::nn::functional::pad(PhiTensor[0], torch::nn::functional::PadFuncOptions({0, maxDim-srctermTensordim[2], 0, maxDim-srctermTensordim[1],0, maxDim-srctermTensordim[0]}).mode(torch::kConstant).value(0));
            PhiTensor[1]= torch::nn::functional::pad(PhiTensor[1], torch::nn::functional::PadFuncOptions({0, maxDim-srctermTensordim[5], 0, maxDim-srctermTensordim[4],0, maxDim-srctermTensordim[3]}).mode(torch::kConstant).value(0));
            PhiTensor[2]= torch::nn::functional::pad(PhiTensor[2], torch::nn::functional::PadFuncOptions({0, maxDim-srctermTensordim[8], 0, maxDim-srctermTensordim[7],0, maxDim-srctermTensordim[6]}).mode(torch::kConstant).value(0));
            ...
            bTensor  =torch::cat({PhiTensor[0],PhiTensor[1],PhiTensor[2]},1);
            SolTensor=torch::cat({umacTensors[0],umacTensors[1],umacTensors[2]},1);
        };
                       ...
                       const std::vector<int> pgTensordims)
{
                ...
                auto custom_dataset = CustomDatasetCNN(RHSCollect,PresCollect,umacCollect,pgCollect,presTensordim, srctermTensordim,umacTensordims,pgTensordims).map(torch::data::transforms::Stack<>());
                ...
                while((lossUx+lossUy+lossUz)>5*e1 and epoch<numEpoch )
                {
                    ...
                    {      // RHS data
                            ...
                            torch::Tensor loss_out =loss_outUx+loss_outUy+loss_outUz;
                ...
                amrex::DistributionMapping dmap, BoxArray  ba)
{
    auto new_function = [func,RefineSol,device,CNN_UX,CNN_UY,CNN_UZ,CNN_P,presTensordim,srctermTensordim,umacTensordims,DivFdim,pgTensordims,dmap,ba](auto&&... args)
    {
        ...
        if (RefineSol==false and step>initNum )
        {
            ...
            int maxU   = *std::max_element(umacTensordims.begin(), umacTensordims.end());
            int maxSrc = *std::max_element(srctermTensordim.begin(), srctermTensordim.end());
            int maxpg = *std::max_element(pgTensordims.begin(), pgTensordims.end());
            int max1   = std::max(maxP,maxU);
            int max2    = std::max(maxpg,maxSrc);
            int maxDim    = std::max(max1,max2);
            ...
            /* Pad RHS tensor array components, and convert to single tensor */
            ...
            for (int d=0;d<AMREX_SPACEDIM;d++) 
            {
                pgNNTensor[d]=pgNNTensor[d].unsqueeze(1);
            }

            /* Pad pg tensor array components, and convert to single tensor */
            pgNNTensor[0]= torch::nn::functional::pad(pgNNTensor[0], torch::nn::functional::PadFuncOptions({0, maxDim-pgTensordims[2], 0, maxDim-pgTensordims[1],0, maxDim-pgTensordims[0]}).mode(torch::kConstant).value(0));
            pgNNTensor[1]= torch::nn::functional::pad(pgNNTensor[1], torch::nn::functional::PadFuncOptions({0, maxDim-pgTensordims[5], 0, maxDim-pgTensordims[4],0, maxDim-pgTensordims[3]}).mode(torch::kConstant).value(0));
            pgNNTensor[2]= torch::nn::functional::pad(pgNNTensor[2], torch::nn::functional::PadFuncOptions({0, maxDim-pgTensordims[8], 0, maxDim-pgTensordims[7],0, maxDim-pgTensordims[6]}).mode(torch::kConstant).value(0));
            torch::Tensor pgTensor  =torch::cat({pgNNTensor[0],pgNNTensor[1],pgNNTensor[2]},1);
            ...
            torch::Tensor Uin1,Uin2,Uin3;
            Uin1= SourceTermTensor.index({Slice(),0,f1_Slice_x,f1_Slice_y,f1_Slice_z})-pgTensor.index({Slice(),0,f1_Slice_x,f1_Slice_y,f1_Slice_z});
            Uin2= SourceTermTensor.index({Slice(),1,f2_Slice_x,f2_Slice_y,f2_Slice_z})-pgTensor.index({Slice(),1,f2_Slice_x,f2_Slice_y,f2_Slice_z});
            Uin3= SourceTermTensor.index({Slice(),2,f3_Slice_x,f3_Slice_y,f3_Slice_z})-pgTensor.index({Slice(),2,f3_Slice_x,f3_Slice_y,f3_Slice_z});

            std::array<torch::Tensor,AMREX_SPACEDIM> umacTensor;
            umacTensor[0] = CNN_UX->forward(Uin1.to(torch::kFloat32),Uin2.to(torch::kFloat32),Uin3.to(torch::kFloat32),
                                maxDim,srctermTensordim,umacTensordims);

            umacTensor[1] = CNN_UY->forward(Uin1.to(torch::kFloat32),Uin2.to(torch::kFloat32),Uin3.to(torch::kFloat32),
                                maxDim,srctermTensordim,umacTensordims);
                                                
            umacTensor[2] = CNN_UZ->forward(Uin1.to(torch::kFloat32),Uin2.to(torch::kFloat32),Uin3.to(torch::kFloat32),
                                maxDim,srctermTensordim,umacTensordims);
        ...
        if(RefineSol == true)
        {
            ...
            if(step<(initNum+TimeDataWindow.size()) and step%retrainFreq==0 and step>0)
            {
                ...
                CNN_TrainLoop(CNN_UX,CNN_UY,CNN_UZ,
                            Unpack_RHSCollect(args...),Unpack_PresCollect(args...),Unpack_umacCollect(args...),
                            Unpack_pgCollect(args...),presTensordim,
                            srctermTensordim,umacTensordims,pgTensordims);
    ...
    while (step < 3000)
    {
        ...
        for (IBMarIter pti(ib_mc, ib_lev); pti.isValid(); ++pti) {
            ...
            for (int i =0; i<np; ++i) {
                ...
                mark.pos(0)=0.5*dis(gen);
                mark.pos(1)=0.5*dis(gen);
                mark.pos(2)=0.5*dis(gen);
                for (int d=0; d<AMREX_SPACEDIM; ++d)
