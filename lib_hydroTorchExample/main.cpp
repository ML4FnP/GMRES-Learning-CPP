...
torch::Tensor read_data_b(torch::Device device)
{
 ...
 // Initialize on GPU
  ...
  // One way of copying over entire array of doubles
...
/* Input: pytorch "device" object (specifies cpu or GPU)
...
/* It's easiest to work with pytorch dataloader objects.
   ...
   pytorch dataset class that we then pass to the dataloader */
...
int main (int argc, char* argv[])
{
    ...
    /* NOTE: Weights and tensors must be of the same type. So, for function calls where input tensors interact with
            weights, the input tensors are converted to  single precision(weights are floats by default). Alternatively we can 
